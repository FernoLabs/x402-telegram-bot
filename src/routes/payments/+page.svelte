<script lang="ts">
  ...
  async function loadPayments(force = false) {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(typeof payload?.error === 'string' ? payload.error : 'Unable to load payments');
      }
  ...
  function resolveStatus(entry: MessagePaymentHistoryEntry) {
    ...
    if (requestStatus === 'expired') {
      return { label: 'Expired', tone: 'danger', description: 'This payment request expired before it was confirmed.' };
    }

    switch (messageStatus) {
      case 'awaiting_payment':
        return { label: 'Waiting for payment', tone: 'warning', description: 'Complete the transaction to publish your message.' };
      case 'signature_saved':
        return { label: 'Signature saved', tone: 'info', description: 'We received your signature and are waiting for confirmation.' };
      case 'paid':
        return { label: 'Payment confirmed', tone: 'info', description: 'Payment was confirmed on-chain. You can resend the message if needed.' };
      case 'sent':
        return { label: 'Message posted', tone: 'success', description: 'The Telegram bot delivered this message.' };
      case 'failed':
        return { label: 'Delivery failed', tone: 'danger', description: entry.message?.lastError ?? 'We could not deliver this message.' };
      default:
        return { label: 'Unknown status', tone: 'neutral', description: 'Status could not be determined.' };
    }
  }
  ...
  async function buildPaymentTransaction(entry: MessagePaymentHistoryEntry) {
    ...
    if (currency === 'SOL') {
      ...
      const payerAta = await findAssociatedTokenPda({ owner: payerAddress, mint, tokenProgram: TOKEN_PROGRAM_ADDRESS });
      const recipientAta = await findAssociatedTokenPda({ owner: recipientAddress, mint, tokenProgram: TOKEN_PROGRAM_ADDRESS });
      ...
      const [payerAccountInfo, recipientAccountInfo] = await Promise.all([
        ...
        rpc.getAccountInfo(recipientAtaAddress, { commitment: 'confirmed', encoding: 'base64' }).send()
      ]);
...
<article class="payments-page">
  ...
  {#if !walletAddress}
    <p class="info">Connect your Solana wallet with the button in the top right to see your payments.</p>
      ...
      {#each payments as entry (entry.request.paymentId)}
        ...
        <li class="payment-card">
            ...
            <div class="card-header">
              ...
              <span class="amount">{entry.request.amount.toFixed(2)} {entry.request.currency?.toUpperCase() ?? 'USDC'}</span>
            </div>
            <p class="status-description">{status.description}</p>
            <section class="details">
              <h4>Message</h4>
              <p class="message-text">{entry.message?.message ?? entry.request.memo ?? 'Message details unavailable.'}</p>
            </section>
            <section class="details">
            ...
            <div class="card-actions">
              {#if canPay(entry)}
                <button type="button" on:click={() => payWithWallet(entry)} disabled={actionBusy(entry)}>
                  Pay with wallet
                </button>
              {/if}
              {#if canVerify(entry)}
                <button type="button" on:click={() => verifySignature(entry)} disabled={actionBusy(entry)}>
                  Check status
                </button>
              {/if}
              {#if canResend(entry)}
                <button type="button" on:click={() => resendMessage(entry)} disabled={actionBusy(entry)}>
                  Resend message
                </button>
              {/if}
            </div>
